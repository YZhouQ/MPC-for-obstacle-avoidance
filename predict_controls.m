function [controls] = predict_controls(plan_horizon,agent_state,goal_state,vp,wp,v0,w0,...
    agent_radius,obstacle_radius,obstacle_pos,obs_v,obs_w,dt)
    ...
    A = [A; [zeros(plan_horizon-1,plan_horizon) -diff(eye(plan_horizon))]];
    b = [amax*dt*ones(plan_horizon-1,1);-amin*dt*ones(plan_horizon-1,1); ...
        alphamax*dt*ones(plan_horizon-1,1);-alphamin*dt*ones(plan_horizon-1,1)];
...
options = optimoptions(@fmincon,'Display','none');
    controls = fmincon(cost,[v0,w0],A,b,[],[],lb,ub,...
        @(controls)nonlcon(controls,plan_horizon,agent_state,agent_radius,obstacle_radius,obstacle_pos,obs_v,obs_w,dt),options);
